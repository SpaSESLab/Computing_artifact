...
source("Prompt_builder_edited.R") # all prompts + extract_* functions
...
# ============================
#df <- read.csv(
  #"/Users/agnesnamyalo/Desktop/RESEARCH/DATA/wildlife_news_articles.csv",
...
final_df_wildlife <- df_test %>%
  mutate(
    baseline_esa    = map_int(Text, baseline_any_esa_keyword),
    baseline_causal = map_int(Text, baseline_contains_cause_word),
    npf             = map(Text, extract_npf),
    story_struct    = map(Text, extract_story_elements),
    event_summ      = map(Text, extract_event_summary)
  ) %>%
  mutate(
...
final_df_wildlife <- final_df_wildlife %>%
  mutate(
...
#write_csv(final_df_wildlife, "narrative_coded_results_with_baselines_widlife_180.csv")
