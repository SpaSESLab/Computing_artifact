...
# Read in the data
wildlife <- read.csv(file = "/Users/agnesnamyalo/Desktop/RESEARCH/DATA/Artifact_congress_for_STM.csv")

head(wildlife, 5)


# Tokenization & cleaning
tokens <- wildlife$cleaned_text %>%
  tokens(what = "word",
         remove_punct = TRUE,
         remove_numbers = TRUE,
         remove_url = TRUE) %>%
...
labels <- labelTopics(model)
# Get top-N (say 10) keywords per topic
news_keywords <- unique(unlist(labels$prob[ , 1:10]))
# Optionally: save this as a vector
