...
pheatmap(
  cosine_mat,
  filename = "/Users/agnesnamyalo/Desktop/RESEARCH/DATA/congress_cosine_similarity_heatmappp.png",  # save here
  display_numbers = TRUE,
  color = colorRampPalette(c("white", "skyblue", "navy"))(100),
  main = "Cosine Similarity between STM and BERTopic Topics",
  fontsize_number = 7,
...
#Trend for STM Topic 4 & 8, BERTopic Topic 2 -- Congressional documents
...
plt <- ggplot(full_df, aes(x = Year, y = Document_Count, color = Method, group = Method)) +
  ...
  labs(
    title = "Trend: STM Topic 4 & 8 vs BERTopic Topic 2 -- Congressional documents",
    x = "Year",
    y = "Document Count"
  )
