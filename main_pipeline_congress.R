...
df <- read.csv(
  ...
  "/Users/agnesnamyalo/Desktop/Research_work/ESA_ALL_new_combined.csv",
...
final_df_congress <- df_test %>%
  mutate(
    baseline_esa    = map_int(extracted_text, baseline_any_esa_keyword),
    baseline_causal = map_int(extracted_text, baseline_contains_cause_word),
    npf             = map(extracted_text, extract_npf),
    story_struct    = map(extracted_text, extract_story_elements),
    event_summ      = map(extracted_text, extract_event_summary)
  ) %>%
  mutate(
...
write_csv(final_df_congress, "congress_document_outputs_180.csv")
